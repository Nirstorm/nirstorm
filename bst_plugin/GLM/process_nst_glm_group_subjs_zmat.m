...
sProcess.options.normalize_with_std.Comment = 'Divide by std-deviation';
sProcess.options.normalize_with_std.Type    = 'checkbox';
sProcess.options.normalize_with_std.Value   =  1;
...
% Loop over full list of ROIs within FOV and fill matrix with mean
% effect within each region
...
for iroi=1:length(roi_indexes)
    ...
    if sum(roi_mask) > 0
        if sProcess.options.normalize_with_std.Value
            mean_std = mean(all_con_std(:, roi_mask), 2);
            if mean_std > 0
                all_roi_z(:, iroi) = mean(all_con(:, roi_mask), 2) ./ mean_std;
            else
                warning('Std is null for ROI %d', iroi);
            end
        else
            all_roi_z(:, iroi) = mean(all_con(:, roi_mask), 2);
        end
    end
end
