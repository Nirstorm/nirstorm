function varargout = process_nst_detect_bad_CV( varargin )
...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.options.option_coefficient_variation.Comment = 'Channel rejection by coefficient_variation';
    sProcess.options.option_coefficient_variation.Type    = 'checkbox';
    sProcess.options.option_coefficient_variation.Value   = 1;
    
    sProcess.options.coefficient_variation.Comment = 'Channel rejection by coefficient_variation';
    sProcess.options.coefficient_variation.Type    = 'value';
    sProcess.options.coefficient_variation.Value   = {15};
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    do_CV_channels = sProcess.options.option_coefficient_variation.Value;
    CV_threshold = sProcess.options.coefficient_variation.Value{1};
...
function [channel_flags, removed_channel_names] = ...
    Compute(nirs_sig, channel_def, channel_flags, do_remove_neg_channels, ...
            max_sat_prop, invalidate_paired_channels, nirs_chan_flags, CV_threshold, do_CV_channels) %#ok<DEFNU>   
...
%  
    ...
    if do_CV_channels
       CV = std(nirs_sig,1)./mean(nirs_sig,1).*100;
       bad_channels = any(CV>CV_threshold);
       channel_flags(bad_channels) = -1;
    end
